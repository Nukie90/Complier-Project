...
tokens = (  # Define tokens FIRST
    ...
    "ADD", "SUB", "MUL", "DIV", "EXP",  # ADD EXP TOKEN
    "EQUALS", "NE",
...
precedence = (
    ('left', 'ADD', 'SUB'),
    ('left', 'MUL', 'DIV'),
    ('right', 'EXP'),  # Exponentiation should be evaluated right to left
)
...
def p_expression_binop(p):
    ...
    if isinstance(p[1], (int, float)):
        ...
        p[0] += f"LD R0 @{p[1]}\n"
        left_type = type(symbol_table[p[1]][0])
    else:
        left_type = None
    ...
    if isinstance(p[3], (int, float)):
        ...
        p[0] += f"LD R1 @{p[3]}\n"
        right_type = type(symbol_table[p[3]][0])
    else:
        right_type = None

    if p[2] == "^":  # Handle exponentiation separately
        if left_type is float or right_type is float:
            p[0] += "FL.i R0 R0\n" if left_type is int else ""
            p[0] += "FL.i R1 R1\n" if right_type is int else ""
            p[0] += "EXP.f R2 R0 R1\nST @print R2"
        else:
            p[0] += "EXP.i R2 R0 R1\nST @print R2"
    else:
